...
ui <- fluidPage(
  fluidRow(
    ...
    column(3,
           sliderInput("range_time", "Construction Year", sep = "", min = 1960, max = 2021,
                       value = c(1970, 2020)),
           sliderInput("range_area", "Area", sep = "", min = 0, max = 350,
                       value = c(0, 200)), )),
  tabsetPanel(
    tabPanel("Chart",
...
# 회귀분석
  output$regression <- renderPlot({
    ...
    grob_1 <- grobTree(textGrob(paste0("All: ", coef_all), x=0.05, y=0.84, hjust=0,
                                gp=gpar(col="blue", fontsize=13)))
    grob_2 <- grobTree(textGrob(paste0("Sel: ", coef_sel), x=0.05, y=0.95, hjust=0,
